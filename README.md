...
and then update `COMMENT_WITH_USER_DATA` from statis data.
...
*The easyst way to get current user list from bugzilla is a copy list for `cc`
on New Bug page
...
1. **Name of image** and **description for attachment** must
  ...
  Username is a part of email before `@`. Attantion! `Username !== real_name`
2. You must to use only **`jpg`** format for images
3. The image must be square
4. The image must be not more of 10 kb size*

*Size of image is importante because bugzilla can not to send
